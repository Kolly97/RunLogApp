...
- **Methodik** (eigene Seite, v1.6.0) — **N-of-1 Methoden-Findung:** aktuelle Marker-Batterie (Critical Speed primär, VDOT, Threshold-Pace/HF, aerobe Entkopplung, Submax-EF, Effective VO2max, Laktat-an-Pace, Polarisierungs-Index), passive Inferenz (welches Wochen-Regime mit Fortschritt korreliert — polarisiert/pyramidal/threshold/Norwegian, mit Confounder-Kontrolle) und geführte Methoden-Experimente mit Vorher/Nachher-Auswertung (Verdikt aus CS + Konfidenz, kleine n als „explorativ" markiert). Korrelation, nicht Kausalität.
- **Saisonplan** (eigene Seite) — Wochen anlegen/bearbeiten; automatisch nach Datum sortiert und ab 0 nummeriert.
- **Profil** — Athletenprofil (Geburtsjahr, Geschlecht, Gewicht, Max-HF für VO2max-Norm), HF-Zonen Lauf + **Fahrrad** separat, Pace- und Power-Zonen. **Laktat-/Feldtest-Diagnostik** (v1.3.0): Stufentest eingeben → automatische LT1/LT2-Berechnung (HF + Pace) → Zonen-Set-Vorschlag + Schwellen-Trend über Zeit. **Trainings-Verfügbarkeit** (v1.4.0): Zeitbudget je Wochentag, Longrun-Tag, Qualitätstage, Doubles. **VO2max-Laborwerte + Ruhe-HF** (v1.5.0): mehrere Labormessungen über Zeit eichen die Effective-VO2max-Schätzung. **Block-Präferenzen** (v1.8.0): Schwerpunkt (Schwelle/VO2/Berg/Norwegian/Fartlek) + Lieblings-/Vermeiden-Einheiten steuern den Trainingsplan-Inhalt (beratend); aufgeräumte Bereichs-Navigation; **Tutorial-Profil** mit komplettem Beispieljahr zum Ausprobieren. Profile umbenennen/löschen/zurücksetzen.
- **Einstellungen** — Zonen-Sets mit Gültig-ab-Datum, Analyse-Schwellen, **adaptiver Coach (Readiness-Gate advisory/gate)** (v1.5.0), Strava-Verbindung mit **Anreicherungs-Fortschritt** (Doughnuts Details/Streams).
...
## Änderungshistorie
Siehe [CHANGELOG.md](CHANGELOG.md) — aktuell **v1.8.0**.
