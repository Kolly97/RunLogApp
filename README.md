...
## Architektur
- `server/` — Express + `node:sqlite`. `db.ts` (Schema/Migrationen/Profile), `load.ts` (TSS/PMC/NGP/Best-Efforts/Zonen-Splits), `analysis.ts` (Prüf-Engine + Plan-Erfüllung + Engine), `zones.ts` (Zonen-Sets + LT1-Anker), `lactate.ts` (LT1/LT2-Algorithmus, pure), `strava.ts` (OAuth/Sync/Zonen-Import), `index.ts` (API + statisches Hosting).
...
## Änderungshistorie
Siehe [CHANGELOG.md](CHANGELOG.md) — aktuell **v1.3.0**.
